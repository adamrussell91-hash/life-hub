...
test('returns projectRelationshipRegistry output and omits duplicate_fields and storage details', async () => {
  ...
  assert.equal(body.data.relationships.length, 24);
  ...
  assert.ok(relatedTo);
  assert.equal(relatedTo.inverse_label, 'related_to');
  assert.deepEqual(relatedTo.metadata_keys, ['migration_source']);
