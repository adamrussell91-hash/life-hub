...
test('the Fitness tab renders the fixture workout and labeled analytics', async () => {
  ...
  try {
    ...
    assert.equal(await page.locator('[data-fitness="week-pips"] .fitness-pip').count(), 4);
    assert.match(await page.locator('[data-fitness="recovery-count"]').textContent(), /0/);
    ...
    assert.equal(await page.locator('#fitness-dashboard .metric-ring').count(), 0);
    assert.equal(await page.locator('#fitness-dashboard .line-chart').count(), 0);
    assert.equal(await page.locator('#fitness-dashboard .meal-protein-pie').count(), 0);
    assert.equal(await page.locator('#fitness-rep-card').isVisible(), true);
    assert.match(await page.locator('#fitness-rep-bars').textContent(), /reps|set/);
