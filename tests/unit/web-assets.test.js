...
test('phone Chat bubbles match Messenger body density (16px / tight leading)', async () => {
  const css = await readFile(new URL('../../apps/life/css/app.css', import.meta.url), 'utf8');
  assert.match(
    css,
  ...
  assert.doesNotMatch(
    css,
    /\.chat-message--assistant \.chat-message__body\s*\{[^}]*font-size:\s*var\(--text-md\)/
  );
});
...
test('service worker paints cached images immediately and keeps scripts network-first', async () => {
  ...
  // The version number bumps on every shell-asset change — pin the pattern, not a version.
  assert.match(worker, /life-hub-shell-v\d+/);
});
