...
test('the tool schema requires name, servingDescription, core macros, and sodium', () => {
  const schema = foodLibraryEntrySchema();
  assert.equal(schema.name, 'save_food_library_entry');
  assert.deepEqual(schema.input_schema.required, [
    'name', 'servingDescription', 'calories', 'protein_g', 'fat_g', 'sodium_mg'
  ]);
});
...
test('validateFoodLibraryEntry accepts a well-formed entry and trims strings', () => {
  const entry = validateFoodLibraryEntry({
    ...
    calories: 250, protein_g: 11, fat_g: 12, sodium_mg: 640, omega3: 'none'
  });
...
test('validateFoodLibraryEntry rejects a missing required field including sodium', () => {
  ...
  assert.equal(validateFoodLibraryEntry({
    name: 'Toast', servingDescription: '1 slice', calories: 100, protein_g: 3, fat_g: 1
  }), null);
...
test('validateFoodLibraryEntry rejects a non-numeric macro or an invalid omega3 level', () => {
  assert.equal(validateFoodLibraryEntry({
    name: 'Toast', servingDescription: '1 slice', calories: 'lots', protein_g: 3, fat_g: 1, sodium_mg: 100
  }), null);
  assert.equal(validateFoodLibraryEntry({
    name: 'Toast', servingDescription: '1 slice', calories: 100, protein_g: 3, fat_g: 1, sodium_mg: 100, omega3: 'extreme'
  }), null);
});

test('upsertFoodLibraryEntry replaces an existing entry matched by brand+name rather than duplicating it', () => {
  const existing = [
    { name: 'Meatlovers Pizza', brand: 'Domino\'s', servingDescription: '1 slice', calories: 200, protein_g: 9, fat_g: 10, sodium_mg: 500, verifiedAt: '2025-01-01' },
    { name: 'Long Black', servingDescription: '1 cup', calories: 5, protein_g: 0, fat_g: 0, sodium_mg: 5, verifiedAt: '2025-01-01' }
  ];
  const updated = upsertFoodLibraryEntry(
    existing,
    { name: 'Meatlovers Pizza', brand: 'Domino\'s', servingDescription: '1 slice', calories: 250, protein_g: 11, fat_g: 12, sodium_mg: 640 },
    '2026-08-03'
  );
...
test('upsertFoodLibraryEntry appends when there is no existing match', () => {
  const updated = upsertFoodLibraryEntry([], {
    name: 'Toast', servingDescription: '2 slices', calories: 180, protein_g: 6, fat_g: 4, sodium_mg: 220
  }, '2026-08-03');
  assert.equal(updated.length, 1);
  assert.equal(updated[0].verifiedAt, '2026-08-03');
});

test('formatFoodLibraryForPrompt summarises entries with brand, serving, macros including sodium, and verification date', () => {
  const text = formatFoodLibraryForPrompt([
    {
      name: 'Meatlovers Pizza', brand: 'Domino\'s', servingDescription: '1 slice',
      calories: 250, protein_g: 11, fat_g: 12, carbs_g: 24, sodium_mg: 640, calcium_mg: 80,
  ...
  assert.match(text, /sodium_mg=640/);
  assert.match(text, /calcium_mg=80/);
