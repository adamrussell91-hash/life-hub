...
test('Home re-renders with Clare Later and stress-flag extras once Tasks APIs land', async () => {
  const laterTask = {
  ...
  const state = harness({
    tasksApi: {
      async listTasks() { return [laterTask]; },
      async loadStressFlags() { return flags; }
    }
  });
...
test('a missing stress-flags route still paints Home with task extras', async () => {
  const laterTask = {
    title: 'Someday reading',
    kind: 'task',
    priority: 'low',
  ...
  const state = harness({
    tasksApi: {
      ...
      async loadStressFlags() {
        throw Object.assign(new Error('Tasks request failed'), { status: 404, code: 'not_found' });
      }
    }
  });
  ...
  assert.deepEqual(state.calls.lastHomeModel.source.tasks, [laterTask]);
  assert.deepEqual(state.calls.lastHomeModel.source.stressFlags, []);
});
