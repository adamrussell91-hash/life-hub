...
15. `card-swipe.css` + `js/card-swipe.js` — horizontal deck (drag, dots, arrows). Workout session uses `createCardSwipe({ fluid: true })` and appends the exercise card. Copy `snippets/card-swipe.html` or call `createCardSwipe`. Tokens only — no React / shadcn / Hugeicons.
16. `snippets/` — copy the HTML, then wire behaviour (`shell.html`, `rail.html`, `mobile-chrome.html`, `hub-utilities.html`, `sign-in.html`, `sign-in.js`, `confirm-card.html`, `hub-kinetic.html`, `morphing-popover.html`, `card-swipe.html`)
