...
export function buildChatView(): HTMLElement {
  ...
  const field = el('div', 'hub-ai-bar__field');
  const input = el('textarea', 'chat-input hub-ai-bar__input') as HTMLTextAreaElement;
  input.id = 'chat-input';
  input.name = 'message';
  input.required = true;
  input.rows = 2;
  ...
  send.id = 'chat-send';
  send.type = 'submit';
  const tools = el('div', 'chat-form__tools hub-ai-bar__tools');
  tools.append(send);
  const aside = el('div', 'hub-ai-bar__aside');
  const attachList = el('div', 'chat-attach-list');
  attachList.id = 'chat-attach-list';
  attachList.hidden = true;
  aside.append(attachList, tools);
  form.append(label, field, aside);
  view.append(form);
