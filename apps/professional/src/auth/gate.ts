...
export function messageForSignInFailure(err: unknown): string {
  if (err instanceof ApiClientError) {
    ...
    if (err.code === 'forbidden') {
      return 'This origin is blocked.';
    }
    if (err.code === 'usage_exceeded') {
      return 'Sign-in is paused: the API host is over its usage limit. Try again shortly.';
    }
...
/** Passphrase gate from design-kit/snippets/sign-in.html + sign-in.css. The
 * only Professional-specific text is the brand eyebrow. */
  ...
  form.addEventListener('submit', async (event) => {
    ...
    try {
      ...
      if (!confirmed.authenticated) {
        showError('Safari blocked the session cookie.');
        return;
      }
