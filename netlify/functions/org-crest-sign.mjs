import { parseOrgCrestSignRequest } from './_shared/org-crest-sign.mjs';
...
export const config = { path: '/api/organisations/crest-sign' };

export function createOrgCrestSignHandler(deps = {}) {
  return createSessionOriginHandler(async (request, context) => {
    ...
    const parsed = parseOrgCrestSignRequest(parsedBody.value);
    if (parsed.error) {
    ...
    try {
      ...
      return withCors(
        errorResponse(
          ...
          status === 503 ? 'Crest storage is not configured' : 'Crest sign failed',
          status >= 500
        ),
        request,
        env
...
export default createOrgCrestSignHandler();
