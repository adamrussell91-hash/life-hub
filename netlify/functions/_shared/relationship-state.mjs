/**
 * Relationship Activity State classifier (Phase 1, Feature 1.6) — Netlify
 * Functions port.
 *
 * KEEP IN SYNC with `apps/professional/src/domain/relationship-state.ts`.
 * This repo has no shared layer between the Netlify Functions runtime
 * (plain `.mjs`, no bundler) and the `apps/professional` browser bundle
 * (TypeScript, Vite), so the same algorithm/constants/reason-text shape is
 * duplicated here deliberately. Any change to one must be mirrored in the
 * other — see `tests/unit/relationship-state.test.js` (this port) and
 * `apps/professional/tests/unit/relationship-state.test.ts` (the original).
 *
 ...
 *
 * @typedef {'active' | 'cooling' | 'dormant' | 'reactivated' | 'new'} RelationshipState
 *
 * @typedef {Object} RelationshipStateInput
 * @property {string | null} lastMeaningfulInteraction ISO timestamp of the most recent meaningful interaction, or null if none recorded.
 * @property {string | null} previousMeaningfulInteraction ISO timestamp of the meaningful interaction immediately before `lastMeaningfulInteraction`, or null. Needed to detect "reactivated" — a long silence followed by a recent interaction.
 * @property {string | null} upcomingInteraction ISO timestamp of the next scheduled interaction (meeting/event), or null.
 * @property {number} activeSharedContexts Count of currently-open shared contexts (shared projects, active professional_relationship links, etc).
 * @property {string} personCreatedAt ISO timestamp the Person record was created.
 * @property {string} [now] The instant to classify against — defaults to `new Date().toISOString()` if omitted, but MUST be injectable for deterministic tests.
 *
 * @typedef {Object} RelationshipStateResult
...
// data before this ships to users — see BUILD-PLAN.md Feature 1.6. MUST
// match the TS original exactly.
...
/**
 * @param {RelationshipStateInput} input
 * @returns {RelationshipStateResult}
 */
export function classifyRelationshipState(input) {
  ...
  if (hasRecentInteraction || hasUpcoming || hasSharedContexts) {
    ...
    if (hasUpcoming) {
      const daysUntil = Math.round(daysBetween(nowMs, Date.parse(input.upcomingInteraction)));
