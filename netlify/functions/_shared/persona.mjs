...
export function buildSystemPrompt({
  ...
  const stableParts = [...SHARED_STABLE];
  const volatileParts = [];

  if (foodLibrary) {
    stableParts.push(FOOD_LIBRARY_WITH_DATA_PREFIX);
    volatileParts.push(`Food Library:\n${foodLibrary}`);
  } else {
    stableParts.push(FOOD_LIBRARY_FALLBACK);
  }

  if (constraints) {
    stableParts.push(`Standing medical and dietary constraints:\n${constraints}`);
  }

  if (thinCentralNodeLog) {
    volatileParts.push(
  ...
  if (digest) volatileParts.push(`Recent context:\n${digest}`);
  ...
  if (slug === ROUTER_SLUG) {
    ...
    return { stable: joinParts(stableParts), volatile: joinParts(volatileParts) };
  }
  ...
  if (slug === 'chadwick') {
    ...
    if (chadwickProtocol) {
      stableParts.push(`Chadwick operating manual (follow these Life Hub rules; ignore any Notion database mechanics):\n${chadwickProtocol}`);
    }
    if (workoutTemplates) {
      volatileParts.push(`Saved workout templates (living prescriptions — use when Adam says do X again):\n${workoutTemplates}`);
    }
    if (exerciseLibrary) {
      volatileParts.push(
  ...
  if (slug === 'hyaluronica') {
    if (hyaluronicaProtocol) {
      stableParts.push(`Hyaluronica operating manual (follow these Life Hub rules):\n${hyaluronicaProtocol}`);
    }
    if (skincareRoutines) {
      volatileParts.push(
      ...
      stableParts.push('When Adam asks what is on AM/PM, call list_skincare_routines. Never invent a routine from shelf status, in_use flags, or notes keyword search.');
    }
    stableParts.push(
  ...
  if (slug === 'penelope') {
    if (penelopeProtocol) {
      stableParts.push(`Penelope operating manual (follow these Life Hub rules):\n${penelopeProtocol}`);
    }
    stableParts.push(
    ...
    if (mindDiaryDigest) volatileParts.push(`Mind diary digest:\n${mindDiaryDigest}`);
    if (mindSilence) volatileParts.push(mindSilence);
    if (onThisDay) volatileParts.push(`On this day (his own past writing — you may open with it):\n${onThisDay}`);
    if (daysSinceLastEntry != null) volatileParts.push(`Days since last diary entry: ${daysSinceLastEntry}.`);
  }

  if (slug === 'vera') {
    if (veraProtocol) {
      stableParts.push(`Vera operating manual (follow these Life Hub rules):\n${veraProtocol}`);
    }
    if (veraIntake) {
      stableParts.push(`Psychological baseline (longitudinal portrait, not a diagnosis; use it as standing context, do not quote it back at length):\n${veraIntake}`);
    }
    stableParts.push(
    ...
    if (mindTodaySession) volatileParts.push(`Today's mind_session:\n${mindTodaySession}`);
    if (mindDiaryDigest) volatileParts.push(`Mind diary digest:\n${mindDiaryDigest}`);
    if (mindSessionDigest) volatileParts.push(`Mind session digest:\n${mindSessionDigest}`);
    if (workingModelDigest) volatileParts.push(workingModelDigest);
    if (mindSilence) volatileParts.push(mindSilence);
    if (mindDivergence) volatileParts.push(mindDivergence);
    if (daysSinceLastMindSession != null) volatileParts.push(`Days since last Vera session: ${daysSinceLastMindSession}.`);
  }

  if (slug === 'brisket') {
    if (brisketProtocol) {
      stableParts.push(`Brisket operating manual (follow these Life Hub rules; ignore any Notion database mechanics):\n${brisketProtocol}`);
    }
    if (bodyState) {
      volatileParts.push(
  ...
  if (slug === 'sara') {
    if (saraProtocol) {
      stableParts.push(`Sara operating manual (follow these Life Hub rules):\n${saraProtocol}`);
    }
    stableParts.push(
  ...
  if (slug === 'hammond') {
    if (hammondProtocol) {
      stableParts.push(`Hammond operating manual (follow these Life Hub rules):\n${hammondProtocol}`);
    }
    if (hammondAuditContract) {
      stableParts.push(`Hammond audit phase contract (hard rules for this turn):\n${hammondAuditContract}`);
    }
    if (centralNodeFull) {
      volatileParts.push(`Full Central Node (complete markdown — your primary coordination document this turn):\n${centralNodeFull}`);
    }
    if (hammondDigest) {
      volatileParts.push(`Logging history (90-day path presence per domain, plus a real fitness streak from completed/planned/skipped classification — you are the one place in the system that reads this full history):\n${hammondDigest}`);
    }
    if (hammondCnSummary) volatileParts.push(hammondCnSummary);
    if (governanceLogTail) {
      volatileParts.push(`Governance Log (recent tail — durable protocol notes and Coach's Notes):\n${governanceLogTail}`);
    }
    if (governanceLogIsEmpty) {
      volatileParts.push('Governance Log is empty. Once this turn, mention that Notion carried two open items forward — drift: "Build a life worth enjoying" (Still Active as of 9 Jul); escalation: August study load (past its 15 Aug checkpoint) — and ask Adam how to handle them (close / carry / drop). Do not invent more carried-over items. After the first append_governance_log succeeds this instruction stops firing.');
    }
    if (mindSilence) volatileParts.push(mindSilence);
    if (hammondDiaryDigest) {
      volatileParts.push(`Mind diary digest (5e/6b this turn — metadata only, do not quote prose):\n${hammondDiaryDigest}`);
    }
    if (hammondMindAmbient) volatileParts.push(hammondMindAmbient);
    stableParts.push(
