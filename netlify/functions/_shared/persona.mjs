...
export function buildSystemPrompt({
  ...
  const chadwickBlocks = slug === 'chadwick' ? [
    ...
    typeof daysSinceLastSession === 'number'
      ...
      : '',
    lastWorkouts
      ? `Recent sessions (a bounded recent window of fitness files — not full history). "Last completed" is the answer to when Adam last trained and what it was — never use a Planned row for that. Planned rows are today's prescription, not history. If Last completed is present, answer from it (or call get_last_workout for sets). search_workout_records only sees this recent window. Never guess or claim you cannot see history when Last completed is listed.\n\n${lastWorkouts}`
      : '',
