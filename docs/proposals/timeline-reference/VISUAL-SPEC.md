...
| `golden/*.png` | Target screenshots at 2× scale. `timeline-1280`, `timeline-390`, `week-1280`, `term-1280`, `critical-1280`, `drag-1280`, `hammond-1280`, `lines-1280`, `lines-390`, and `focus-forecast-1280` (Focus and Forecast on). `morph-midflight-1280` is illustrative only (timing-dependent, not pixel-compared). |
...
| Part | Spec |
|---|---|
| Toolbar | Left: zoom pills (Year, Term, Month, Week, Day) then view pills (Bars, Lines), both kit `.hub-pills` with the sliding thumb. Right: toggles Critical path, Load, Holidays, Today, **Focus**, **Forecast** (`.tl-toggle`, pressed state uses the kit pressed colours; Critical path pressed uses `--high-sea-ink` on a 14 % High Sea wash), then **Ask Hammond** (`.btn--secondary`), then the 40px round add button. Focus and Forecast are off by default. Forecast is disabled, at 45 % opacity, with tooltip “Needs 20 finished tasks with an estimate and an actual”, until that many finished tasks have both values. While Focus is on, the zoom pills dim to 45 % and ignore clicks, wheel and +/-. |
...
| Focus lens | Optional. A 14-day window at Day width (96px), 21 days either side at Week width (34px), everything else at Term width (4.4px). Holiday compression still applies in the shoulders and outside; inside the window every day stays 96px, including a holiday, so the fortnight is fourteen equal columns. Wash: Wave 8 %, behind the bars, from the top of the canvas to the bottom of the rows, with 1.5px Wave edges. Range pill sits just under the axis and stays at the visible left edge of the window while you scroll (the window is wider than the screen): paper, 1px `--line`, height 20, rx 10, “dd/mm/yy – dd/mm/yy” 12px/600 `--navy`, grab cursor. `[data-part="lens"]` carries `data-start` and `data-end` (inclusive). Dragging the pill moves the window by one day per 96px and keeps the left edge under the pointer; Arrow keys move one day. Turning Focus on snaps the window to the Monday of today (21/09/26 when the clock is 22/09/26). Shoulders have no extra fill. Bars that cross a boundary stretch continuously. |
| Forecast tails | Optional. For each open task with an estimate, a hatched tail from the planned end (the bar's right edge, or the marking shadow's return date) to the P85 finish. P85 is the nearest-rank percentile of that domain's actual÷estimate ratios: index `ceil(0.85 × n) − 1`. Extra days are `(estimate × (ratio − 1)) / 120`, fractional when under a day. A milestone has no estimate; its tail is `(P85 − 1) × 5` days, the domain overrun on one school week, starting just to the right of the diamond. Hatch is domain colour, 7px pitch, rotated −30°, 1.5px line at 50 % on a 12 % wash, with a 1px domain stroke at 40 %. It is not the wall hatch (navy, +45°, 8px). The tail sits behind the bar face. `[data-part="forecast-tail"]` with `data-task-id` or `data-milestone-id`, `data-domain` and `data-days`. The reference uses 24 prototype ratios (not `fixture.json`) so the toggle can be shown: teaching P85 1.95, professional 1.80, life 1.70. |
...
| Layer order (back to front) | grid and holidays, walls, axis, focus lens wash, today, goal bands and brackets, curves, marking shadows, forecast tails, bars (tasks, milestones, projects, dreams, chips, ribbons), lens handle, ghosts and ripples, load strip. |
...
| Focus on / off | The scale becomes piecewise or returns to the zoom; the lens wash settles in or out | `MOTION.settle` 420 | EASE | snaps to the Monday of today when turned on | Zoom is ignored until Focus is off | Instant |
| Drag the lens | The window follows the pointer 1:1 and snaps to a day | per frame (`place`) | none | none | n/a | Same |
| Forecast on / off | Tails settle out from the planned end, or leave | 420 in, 160 out | EASE | none | n/a | Instant |
