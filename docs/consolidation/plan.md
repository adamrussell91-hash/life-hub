...
**Risk (2026-09-04):** CORS hardened in place — exact-match `https://adamrussell91-hash.github.io` (plus `ALLOWED_ORIGINS` / `ALLOWED_ORIGIN`, ignoring `*`). `ai` allowlists models and caps `max_tokens` at 2000. Still a shared OpenAI key; do not fold onto `life-hub2` until widget consumers migrate.
