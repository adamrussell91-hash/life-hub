...
Limits: each intake/text value 12000 characters, request 64000 bytes, session 1000000 bytes, transcript 500 turns, at most 50 sessions listed. Provider output limits are softer ceilings with rewrite-then-trim; a session never fails for length alone. Stale queued/running work becomes retryable after the runner lease expires. Background invocations must reach the same API host.
