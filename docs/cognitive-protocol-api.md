...
`GET /api/knowledge/protocols?list=1` → `{ok:true,data:{sessions:SessionSummary[]}}` with optional `limit` and `offset` paging. Each summary includes `title` and `summary` when present. There is no hard 50-run cap.
...
`POST /api/knowledge/protocols/run` is the authenticated server background runner; UI must not invoke it. Create and action POSTs kick that runner and return 202 while queued. A run processes stages until a user checkpoint, pause, completion or visible failure. Poll the session every 400 ms while queued/running. Back off after request failures; stop polling on disposal and terminal/checkpoint states. If the background kick fails, the same POST falls back to an inline run and returns 200.
...
type Action = 'answer'|'confirm'|'decline'|'correct'|'uncertain'|'finish'|'reflect'|'pause'|'resume'|'cancel'|'retry'|'reopen'|'wrap'|'close';
type Turn = {id:string;role:'voice'|'user'|'controller';speaker:string;stage:string;text:string;createdAt:string;evidenceIds:string[];trimmed?:boolean;done?:boolean;nextSpeaker?:string};
type Evidence = {id:string;kind:'knowledge'|'self-report';title:string;text:string;url?:string};
type Session = {id:string;protocolId:ProtocolId;mode:string;intake:Record<string,string>;revision:number;status:'queued'|'running'|'waiting'|'paused'|'completed'|'cancelled'|'failed';stage:string;speaker:string|null;transcript:Turn[];evidence:Evidence[];evidenceStatus:string;checkpoint:null|{kind:string;question:string};allowedActions:Action[];error:null|{code:string;message:string;retryable:boolean};createdAt:string;updatedAt:string;summary?:{title:string;keyFinding:string;summary:string;openQuestions:string[];forHammond:string|null};writeBack?:{ok:boolean;error?:string;written:{section:string;text:string}[]}};
```

On completion the server stores a `summary` and may append Central Node `recent_actions` / `cross_agent` lines. A failed write-back is recorded on `writeBack` and never fails the run.
Voice JSON may include `done` (boolean, default true). When a burst asks a question and will continue, set `done: false`; the controller re-runs the same step after the answer. When length still exceeds the soft ceiling after two rewrites, the server trims to the last complete sentence, keeps the question, and sets `trimmed: true` on the turn. Length never fails a session.

Render `allowedActions` as authoritative. `answer`, `correct`, `reflect` and `reopen` require nonblank text. `confirm`, `uncertain`, `decline` and `close` are explicit user decisions. `wrap` is Fates-only and offered only at ordinary answer checkpoints before the Fallacy Filter (not at filter, reopen, Weave or close); it jumps to the filter. At the filter checkpoint, `confirm` holds with caution and carries the flagged note into the Weave; `reopen` inserts one reopen stop for the Fate named by the filter turn's `nextSpeaker` (capped at two reopens); `close` skips the Weave caution note but still runs the Weave and close. Confirmation cannot be replaced by model output. `finish` appears only when Consilium has adequate contributions and user responses; it requests the convergence map, whose reflection closes without further analysis. `correct` revises Witness' trace or restarts Mirror/framing from the correction. `pause` and `cancel` may stop a running call from publishing. `resume` restores the previous checkpoint when one existed. `retry` resumes a failed/interrupted stage without repeating committed turns. A provider call interrupted before commit may be reissued; exactly-once billing cannot be guaranteed.
...
1. Fates, design a library event, sprint then normal: count 3 versus 6 alternating stops, one to two bursts per stop, each wait, preamble confirmed, wrap and reopen available where allowed, no Weave before closure. Strip labels: Clotho's fragments/Greek sparks, Atropos' surgical qualification, Lachesis' numbered precision and Weave's balanced witnessing must differ.
2. Horizon, career trajectory without desired future: Alvar explicitly projects with moderate-to-low ceiling; Ketill stays within two years and may ask a steering question; Sigrid asks trade-off/drift/unclassified. No plan.
...
6. Consilium, a competing-duty dilemma: framing waits, Virtue never first, adaptive order, short turns per voice, signature opening only once, questions stop. All voices and user contribute before map. Reflection is final and unanalysed; no verdict.
...
8. Tribunal, a repeated scheduling conflict: independently generated prompts see identical original input/evidence and no other voice; scaling tries down/up; convergence note never ranks. Thin intake may get shared clarify questions first. Try a null reframe.
