...
- Same-origin functions expose only allowlisted, date-bounded repository manifests and exact changed blobs. GitHub tokens, passphrase verifiers, session secrets, raw provider errors, and unrestricted repository access never reach browser responses or assets.
- Explicit logout clears the private repository cache. An authenticated tab can reload its cached read-only view offline before session expiry, while a fresh offline tab remains signed out.
- `npm ci --ignore-scripts`: clean install, 5 packages added and 6 packages audited.
- `npm test`: 169 unit and integration tests passed, 0 failed.
- `npm run test:browser`: 7 Chromium acceptance tests passed, 0 failed, covering rejected and successful sign-in, desktop and 390-pixel Home, incremental refresh, sign-out, session expiry, deterministic response secret checks, and offline gating.
