...
**Implementation note for the Central Node plan:** unlike `config/targets.yml`, neither `config/agents.yml` nor `central-node.md` produces a `missing_*` warning when absent from a sync — both `agentsConfig` and `centralNodeMarkdown` simply resolve to `null` silently. The Central Node tab's render/model layer must explicitly handle `centralNodeMarkdown === null` (first sync, GitHub outage, file renamed upstream) as its own empty/unavailable state, rather than assuming `renderWarnings`'s existing warning panel will ever surface it.
